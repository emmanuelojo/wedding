<template>
  ...
  >
    ...
    >
      <div :class="['glass-card rounded-2xl p-6 md:p-8', isReversed ? 'md:ml-8' : 'md:mr-8']">
        <span class="text-gold-400/60 font-serif text-sm">{{ year }}</span>
        <h3 class="font-serif text-2xl md:text-3xl text-cream-100 mt-2 mb-3">
          {{ title }}
        </h3>
        <p class="text-cream-300/60 text-sm leading-relaxed">
          {{ description }}
        </p>
      </div>
    </div>
